...
utils.error = function(callback, message) {
    if (callback) {
        ...
        // console.trace();
        if (typeof global.it === 'function') // running under mocha?
...
var hasWord = utils.hasWord = R.curry(function(word, str) {
    return str.match(new RegExp("\\b" + word + "\\b"));
});

var hasWords = utils.hasWords = R.curry(function(words, str) {
    return words.every(function(word) {
        return hasWord(word, str);
    });
});

var saveTo = utils.saveTo = R.curry(function(filename, content) {
    // use shelljs .to construct
    content.to(filename);
});
