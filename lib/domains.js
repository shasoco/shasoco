...
var compile = function(conf) {
    return function(filename) {
        _.template(shell.cat('compose/' + filename))(conf).to(projectDir(conf.name) + '/' + filename);
    };
};
...
var create = domains.create = function(name, options) {
    ...
    save(conf);
    shell.ls('compose').forEach(compile(conf));
    return conf;
};
...
var load = domains.load = function(name) {
    return yaml.safeLoad(shell.cat(projectFile(name)));
};
...
var list = domains.list = function(name) {
    // TODO
};
