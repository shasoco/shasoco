...
var SHASOCO_PROJ_DIR = "/var/lib/shasoco/domains"; // process.env.HOME + "/.shasoco/domains";
...
var projectFile = domains.configPath = function(name) {
...
var compose = domains.compose = R.curry(function(conf, args) {
    ...
    var ret = shell.exec('docker-compose ' + args);
    shell.cd(cwd);
    return ret;
});

var dockerExec = domains.dockerExec = R.curry(function(conf, service, cmd) {
    return shell.exec('docker exec -it ' + composeName(conf.name) + "_" + service + "_1 " + cmd);
});
